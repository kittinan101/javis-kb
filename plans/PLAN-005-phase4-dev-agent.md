...
- [ ] **Prerequisite:** ยืนยัน GitHub plan — branch protection + required reviewers บน **private repo ต้องใช้ Team plan ขึ้นไป** (Free ให้เฉพาะ public repo) → ตั้งงบ + ผู้อนุมัติก่อน ไม่งั้น guardrail ชั้นที่ 3 ไม่มีอยู่จริง
...
- [ ] **`develop` ต้อง protect ด้วย (Require PR):** ไม่งั้น agent ที่มีสิทธิ์ git push จะเข้า develop ตรงได้โดยไม่ผ่าน review แล้ว auto-deploy ทันที — กติกา "push แค่ feature/*" ใน CLAUDE.md เป็น soft rule ต้อง enforce ที่ branch protection
- [ ] `CODEOWNERS`: `/prisma/migrations/` → Lead + ผู้รับบท DBA (map ใน Roster — ไม่มี role DBA ใน enum ใช้คนจาก LEAD_SA), `/.github/workflows/` → Lead, `*` → dev-team
- **AC:** ลอง push ตรงเข้า main **และ develop** ด้วย PAT ของ runner → ถูกปฏิเสธทั้งคู่
...
- [ ] ตาราง `JobQueue(id, status: queued|claimed|running|done|failed, payload Json, attempts, claimedAt, createdAt, updatedAt)` ใน javis-core — **ใส่ `@@map`/`@map` ให้ column เป็น snake_case ตรงกับ SQL ด้านล่าง** (Prisma default เป็น camelCase quoted — raw SQL จะพัง) + index `(status, created_at)`
- [ ] n8n เขียนงาน `status=queued` → poke runner ที่ `:8787/poke` (ผ่าน Tailscale, ตรวจ header `x-javis-token`)
- [ ] **ก่อนรับงาน runner ตรวจ approval กับ javis-core DB** — ห้ามเชื่อ `status: approved` ในไฟล์ plan (ไฟล์แก้เองได้ DB คือ source of truth — ดู PLAN-004 T1.3)
...
RETURNING *;
```
- [ ] **Lease renewal แทน timeout คงที่:** runner heartbeat อัพเดต `updated_at` ของงานที่รันทุก 1–5 นาที → n8n cron reset เฉพาะงานที่ lease ขาดจริง (updated_at เก่ากว่า 10 นาที) เป็น queued + attempts+1 — (timeout คงที่ 2 ชม. จะฆ่างานใหญ่ที่ยังรันอยู่ → เกิด 2 instance รันซ้อน/PR ซ้ำ); เกิน 3 ครั้ง = failed + alert
- [ ] จบงาน → update done/failed + callback n8n — n8n cron sweep งาน `done` ที่ยังไม่ notify ด้วย (กัน callback หายตอน n8n ล่มพอดี)
...
- [ ] ลำดับต่องาน: (1) reset workspace ด้วย `reset.sh` (`git reset --hard && git clean -fdx && git checkout develop && git fetch && git reset --hard origin/develop; docker container prune -f && docker image prune -f`) → checkout `feature/{job_id}` (2) รัน claude -p ด้วย plan ที่ approve (3) test ใน Docker arm64: `npm ci && npm run build && npm test` — fail → agent วิเคราะห์+แก้ วนไม่เกิน 3 รอบ (4) `gitleaks git` scan ก่อน commit — เจอ secret = fail งานทันที (5) agent commit ใน workspace → **runner harness เป็นคน push ไป feature branch เท่านั้น** (6) runner `gh pr create` base=develop ใส่สรุปงาน + cost ใน body (7) callback n8n: `{job_id, status, cost_usd, session_id, duration_ms}`
- [ ] คำสั่ง Claude Code ตาม Spec §4 — **ยืนยัน flags กับ docs รุ่นล่าสุดก่อนใช้** (model id เป็น placeholder และ `--max-budget-usd` อาจไม่มีจริง → **budget enforcement หลักอยู่ที่ runner**: อ่าน `total_cost_usd` จาก output JSON แล้วตัดงาน/หยุดรับงานใหม่เมื่อเกิน `SystemConfig.max_budget_per_job_usd`):
```bash
claude -p "$(cat /tmp/job-prompt.md)" \
  --output-format json \
  --allowedTools "Read,Edit,Write,Bash(npm ci:*),Bash(npm run:*),Bash(npm test:*),Bash(npx prisma generate:*),Bash(npx prisma migrate dev:*),Bash(git add:*),Bash(git commit:*),Bash(docker:*),Grep,Glob" \
  --disallowedTools "Bash(npx prisma migrate deploy:*),Bash(git push:*)" \
  --permission-mode acceptEdits \
  --max-turns 40
```
  หมายเหตุจาก review:
  - **agent ไม่มีสิทธิ์ `git push` เลย** — runner harness เป็นคน push + เปิด PR หลัง gitleaks ผ่าน (ปิดรู push ตรง develop ที่ `Bash(git *)` เดิมเปิดไว้)
  - allowlist ต้องมี `prisma migrate dev` (T3.2 ให้สร้าง migration แบบ `--create-only`) — เดิมมีแค่ `generate` ซึ่งขัดกันเอง
  - `--disallowedTools` เป็นแค่ friction ชั้นแรก (bypass ได้เช่น `npm exec`) — ของจริงคือ: PAT ไม่มีสิทธิ์ protected branch + branch protection + **env ของ runner มีเฉพาะ dev DB URL** (secrets uat/prod อยู่ใน GitHub Environments เท่านั้น — เขียนเป็นข้อกำหนด explicit)
  - ไม่ใช้ `--bare` เพราะต้องอ่าน CLAUDE.md
...
- [ ] **Fail-closed:** verdict JSON parse ไม่ได้/field ไม่ครบ = `request_changes` เสมอ ห้าม default เป็น approve
- [ ] **Calibrate evaluator ก่อนเปิด pilot:** ชุด seeded-defect PRs ~10 เคส (missing authz, secret ใน diff, AC ไม่ครบ, test ที่ไม่ assert อะไร) วัด false-approve/false-reject + ระหว่าง pilot บันทึก "% ที่ evaluator เห็นตรงกับ human reviewer" เป็น metric
- [ ] ผล evaluator แปะเป็น comment ใน PR + สรุปในแชท — **มนุษย์ยังต้อง review เสมอ** (evaluator เป็นตัวกรองชั้นแรก ไม่ใช่คนอนุมัติ)
- **AC:** seeded-defect ถูกจับ ≥ 8/10; PR ใส่ SQL injection ปลอม → reject พร้อมชี้บรรทัด; JSON เสีย → request_changes อัตโนมัติ; PR ปกติ → รายงาน AC ครบทุกข้อ
...
- [ ] SEV1 runbook ลง `guides/GUIDE-xxx-incident-runbook.md`: kill-switch → revert PR/deploy → ตรวจ AuditLog ด้วย job_id → postmortem เป็น ADR → เปิดใหม่ — **รวม direct path ที่ไม่พึ่ง n8n** (เผื่อ n8n ล่มพอดี): SSH ผ่าน Tailscale → `UPDATE system_config SET value='false' WHERE key='agent_enabled'` หรือ `launchctl unload`
...
- [ ] **UAT protocol:** entry = smoke test checklist ผ่านบน UAT env; **test cases เป็นของ QA** (review/แก้ AC ที่ generate มาก่อนใช้ — ไม่ใช่รับของ AI มาตรงๆ); exit = ไม่มี defect sev-1/2 ค้าง; timebox 3 วันทำการ เกินแล้วไม่กด = escalate ในแชท
- [ ] เมื่อ deploy UAT สำเร็จ → n8n ส่งข้อความหา role PM_PO + QA พร้อมลิงก์ UAT env + test cases + ปุ่ม Sign-off / Found issues
...
- [ ] บันทึกผู้ sign-off + เวลา + ผลทดสอบ ลง javis-core DB + mirror ท้าย plan doc + `AuditLog`
- **AC:** PO กดคนเดียว → ยังเปิด PR ขึ้น main ไม่ได้; ครบ 2 role → บันทึก sign-off ครบ; Developer ลองกด → ถูกปฏิเสธ; เกิน timebox → escalation เกิดจริง
