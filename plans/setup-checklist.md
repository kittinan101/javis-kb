...
> **บริบทสำคัญ (user แจ้ง 2026-07-19):** งาน implement ทั้งหมดรอบนี้คือ **POC เพื่อนำเสนอ** — ถ้าผ่านจะย้ายเข้า **environment กลางสำหรับทีม** อีกที (NAS ปัจจุบัน = ชั่วคราว) → ผลกระทบ: (1) เป้าจบของ POC = **นำเสนอผ่าน** ไม่ใช่ adoption 50% — M3 อาจย่อเหลือ demo กับทีมเล็กๆ (2) ตัดสินใจ infra แบบ "ย้ายได้เสมอ" — มี n8n exports + prisma migrations + register ครบแล้ว (3) Notion อัพเดตเฉพาะช่วง implement — หยุดเมื่อจบ (4) งานย้าย environment กลาง = แผนใหม่หลังนำเสนอ (ดู register หมวด 5)
...
> 2026-07-19 (คำถาม user ระหว่างเบรค implement): จดทะเบียน **FEAT-007 — IDE Integration (Javis MCP) + Code-grounded Answers** ([features/FEAT-007-ide-integration-code-grounding.md](../features/FEAT-007-ide-integration-code-grounding.md)) — หลักการสำคัญ: Javis เป็น interface, ความรู้ 2 ชั้น = KB docs + **source code จริงจาก repo เดิมของ project (web/API)**; คำตอบเรื่อง code ต้องอ่าน code จริงประกอบ ไม่ใช่ md อย่างเดียว; MCP read-only เท่านั้น; needs ลง register 5.6–5.7 + G1 ใน PLAN-008 ปรับให้ออกแบบ multi-repo ตั้งแต่แรก
...
| 2026-07-19 | 📌 **กติกาใหม่จาก user: อัพเดต Notion status page ทุกครั้งที่สถานะสำคัญเปลี่ยน** — เพิ่มเป็นข้อ 4 ใน standing workflow (.claude/workflow-instructions.md) + ปรับหน้า Notion ให้แสดงงานค้างครบทุก phase (M3 checklist, ค้างรายบุคคล, ของ lead time ยาว) |
| 2026-07-19 | 📌 **สร้างหน้า Project Status ใน Notion** (ใต้หน้า Javis AI) — สรุปสถานะ + ชี้กลับ repo เป็น source of truth กัน docs drift; กติกา: อัพเดตเฉพาะ milestone สำคัญ ไม่ sync รายละเอียด |
